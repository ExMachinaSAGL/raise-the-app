...
import configUtils, { PriorityLevel } from '../../../src/lib/configUtils';

describe('configUtils.ts', () => {
  let mockConfig: {levels: PriorityLevel[]};
  ...
  beforeEach(() => {
    ...
    originalConfig = configUtils.config.levels;
    configUtils.config.levels = mockConfig.levels;
  });

  afterEach(() => {
    configUtils.config.levels = originalConfig; // restore
  });

  it('should return the correct priority level data', (done) => {
  ...
  it('should throw errors when the config is incorrect', (done) => {
